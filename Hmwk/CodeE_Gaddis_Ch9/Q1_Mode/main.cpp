 ...
 * Created on March 25th, 2021, 7:45 PM
 * Purpose: Find modes in dynamically allocated array
 */
...
int *mode(const int *array,int arySize){
    ...
    unsigned short max=1;   //Maximum Frequency of numbers
    unsigned short temp=1;  //Temporary variable used to count
    unsigned short nmodes=0;    //Number of Modes
    unsigned short modePos=2;   //Mode Position
    //Copy the array
    int *ary=copy(array,arySize);
    //Sort the copy
    mrkSort(ary,arySize);
    //Find the max Frequency
    for(unsigned char i=1;i<arySize;i++){
        if(*(ary+i)!=*(ary+(i-1))) {
            if(temp>max&&temp>=2) max=temp;
                temp=1; }
        else temp++; }
    if(temp>max&&temp>=2)max=temp;
    ...
    //Fill the mode array
    modeAry[0]=nmodes;  //set to number of modes
    modeAry[1]=max; //frequency
    //Fill with values that are modes
    temp=1;
    for(unsigned char i=1;i<arySize;i++){
        if(*(ary+i)!=*(ary+(i-1))) {
            if(temp==max) {
                modeAry[modePos]=*(ary+(i-1));
                modePos++; }
                temp=1; }
        else temp++; }
    if(temp==max) modeAry[modePos]=*(ary+(arySize-1));
